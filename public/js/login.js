...
async function handleLogin(e) {
  ...
  btn.disabled     = true; //after first click, prevents user from clicking again
  banner.style.display = 'none';

  try {
    //sends req to server with input 
    ...
    if (data.success && data.token) {
      ...
      // Expires in 7 days
      const expires = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toUTCString(); //set expiry to 7d
